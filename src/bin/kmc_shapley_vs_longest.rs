...
fn main() {
    ...
    let app_config =
        IlloominateConfig::load(format!("{}/{}", data_location, config_filename).as_str())
    ...
    rayon::ThreadPoolBuilder::new()
        ...
        .build_global()
        .expect("TODO: initializing Rayon thread pool failed");
...
fn run_experiment(data_path: &str, app_config: &IlloominateConfig, metric_config: &MetricConfig) {
    ...
    let (mut loo_outputs, mut shapley_outputs, mut longest_session_outputs, mut metric_outputs) =
        create_output_files(data_path, &metric_factory);
    ...
    for seed in [1313] {
        ...
        let kmc_shapley_algorithm =
            KMcShapley::new(shapley_error, max_shapley_num_iterations, seed);
        ...
        let (loo_importances, shap_values, longest_sessions) = if is_vmis {
            ...
            {
                let model: VMISKNN = VMISKNN::fit_dataset(
                    session_train,
                    app_config.hpo.m.unwrap(),
                    app_config.hpo.k.unwrap(),
                    1.0,
                );

                let loo_importances = k_loo_algorithm.compute_importance(
                    &model,
                    &metric_factory,
                    session_train,
                    session_valid,
                );
                let shap_values = kmc_shapley_algorithm.compute_importance(
                    &model,
                    &metric_factory,
                    session_train,
                    session_valid,
        ...
        let num_random_sessions_to_remove =
            *[loo_importances.keys().len(), shap_values.keys().len()]
                .iter()
                .max()
                .unwrap();

        let mut rng = StdRng::seed_from_u64(seed as u64);
        let mut random_keys_to_remove: Vec<u32> = shap_values
            .clone()
            .into_iter()
...
fn write_importance(
    importances: &HashMap<u32, f64>,
    output_file: &mut File,
    used_seed_value: usize,
) {
    let mut indices_sorted_by_importance: Vec<(u32, f64)> =
        importances.clone().into_iter().collect();
    ...
    for (index, importance) in &indices_sorted_by_importance {
        ...
        writeln!(output_file, "{}", data_to_append).expect("Failed to write to file");
    }
}
...
fn create_output_files(
    ...
    let output_file_loo_importances = create_file(
        &format!(
            "{}/__removal_impact_loo_importance_{}_eval_{}.csv",
            data_path, importance_metric_friendly, importance_metric_friendly
        ),
        overwrite_output,
    );
    let output_file_shapley_importances = create_file(
        &format!(
    ...
    let output_file_longest_session = create_file(
        &format!(
            "{}/__removal_impact_longest_session_{}_eval_{}.csv",
            data_path, importance_metric_friendly, importance_metric_friendly
        ),
        overwrite_output,
    );
    ...
    (
        output_file_loo_importances,
        output_file_shapley_importances,
        output_file_longest_session,
        output_files_evaluation_metric_results,
    )
}

fn get_longest_sessions(session_train: &SessionDataset) -> HashMap<u32, f64> {
    let session_lengths: HashMap<u32, f64> = session_train
        .sessions
        .iter()
...
fn read_datasets(data_path: &str, app_config: &IlloominateConfig) -> Datasets {
    match app_config.model.name.to_lowercase().as_str() {
        "vmis" => {
            let session_train: SessionDataset =
                SessionDataset::new(io::read_data(&format!("{}/train.csv", data_path)));
            let session_valid: SessionDataset =
                SessionDataset::new(io::read_data(&format!("{}/valid.csv", data_path)));
            let session_test: SessionDataset =
                SessionDataset::new(io::read_data(&format!("{}/test.csv", data_path)));
            Datasets {
        ...
        "tifu" => {
            ...
            let validation_ratio = app_config
                .nbr
                .as_ref()
