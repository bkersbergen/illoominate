...
pub use crate::baselines::ExperimentPayload;
use crate::importance::{evaluate_dataset};
use crate::importance::mc_utils::{mean_tolerance_score_dataset, permutation};
...
pub fn tmc_shapley(
    ...
    let (bootstrap_mean_score, bootstrap__tolerance) = if truncation_enabled {
        ...
        mean_tolerance_score_dataset(&model, &experiment_payload.metric_factory, &experiment_payload.valid_dataset)
    } else {
        (0.0, 0.0)
    };
...
pub fn one_iteration_dataset(
    ...
    let permutation: Vec<SessionId> = permutation(&experiment_payload.train_dataset, experiment_payload.seed, iteration);
